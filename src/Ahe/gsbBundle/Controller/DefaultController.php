        ...
        if ($request->getMethod() == 'POST' && $form->handleRequest($request)->isValid()) 
        {
                ...
                switch($profil) {
                    case 0 :
                        ...
                        $repository = $this->getDoctrine()->getManager()->getRepository('AheGsbBundle:Comptable');
                        
                        // Fonction définie dans ComptableRepository.php -
                        $comptable = $repository->findByLoginAndMdp($login,$mdp);
                        if($comptable === NULL){
                            $message = $this->get('session')->getFlashBag()->add('erreurLogMdp',"Votre login 
                            ...
                            // sauvergarder les infos du comptable dans une variable de session pour usage dans d'autres fonctions
